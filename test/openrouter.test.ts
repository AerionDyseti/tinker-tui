...
function mockArtifact(
    overrides: { kind?: ArtifactKind; content?: string } = {},
): SessionArtifact {
...
describe("OpenRouterProvider", () => {
    ...
    describe("info", () => {
        ...
        test("uses default capabilities for unknown models", () => {
            ...
            expect(unknownProvider.info.capabilities.maxContextTokens).toBe(
                32000,
            );
        });
    });
    ...
    describe("translateArtifactKind", () => {
        ...
        test("maps agent_response to assistant", () => {
            expect(provider.translateArtifactKind("agent_response")).toBe(
                "assistant",
            );
        });

        test("maps system_instruction to system", () => {
            expect(provider.translateArtifactKind("system_instruction")).toBe(
                "system",
            );
        });

        test("maps knowledge_reference to system", () => {
            expect(provider.translateArtifactKind("knowledge_reference")).toBe(
                "system",
            );
        });

        test("maps tool_use to assistant", () => {
            expect(provider.translateArtifactKind("tool_use")).toBe(
                "assistant",
            );
        });
    ...
    describe("complete", () => {
        test("sends correct request format", async () => {
            ...
            const context = mockContext([
                messageToContextItem(
                    mockMessage({ kind: "user_input", content: "Hello" }),
                ),
            ]);
            ...
            expect(
                (capturedBody!.messages as Array<{ role: string }>)[0].role,
            ).toBe("user");
        });
        ...
        test("streams content chunks", async () => {
            ...
            const context = mockContext([
                messageToContextItem(
                    mockMessage({ kind: "user_input", content: "Hi" }),
                ),
            ]);
        ...
        test("throws on API error", async () => {
            // Use a provider with retries disabled for this test
            ...
            await expect(async () => {
                for await (const _ of noRetryProvider.complete(context)) {
                    // consume
                }
